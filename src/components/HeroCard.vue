<template>
  ...
  >
    ...
    <div v-if="!compact" class="absolute top-0 right-0 z-10">
      <div v-if="hero.rarity == 5" class="bg-yellow-500 text-white text-xs px-2 py-0.5 rounded-bl-lg font-bold">S</div>
      <div v-else-if="hero.rarity == 4" class="bg-purple-500 text-white text-xs px-2 py-0.5 rounded-bl-lg font-bold">A</div>
      <div v-else class="bg-blue-500 text-white text-xs px-2 py-0.5 rounded-bl-lg font-bold">B</div>
    </div>

    <!-- Cost (Top Left) -->
    <div v-if="!compact" class="absolute top-0 left-0 z-10 bg-black/60 text-white text-xs px-1.5 py-0.5 rounded-br-lg">
      {{ hero.cost }}C
    </div>
    ...
    >
      ...
      />
      <!-- Camp overlay -->
      <div v-if="!compact" class="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/90 via-black/60 to-transparent pt-6 pb-1 px-1.5">
